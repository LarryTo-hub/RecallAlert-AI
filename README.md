...
```
FDA/USDA Sources -> Fetcher -> SQLite Store -> Gemini Agent -> WebSocket
                         (Parsing + Matching)
                           |
                           -> React Frontend
                             (User Dashboard)
...
1. **Background polling loop** fetches recalls from FDA and USDA every 60 minutes. FDA uses openFDA pagination for complete coverage and supports incremental refreshes.
...
- **Full FDA coverage** — pagination support across openFDA records, not only first-page web rows
...
**Frontend Stack:**
- **React** — Web dashboard included in `web/`
...
│   ├── store.py            # Recall persistence (SQLite)
...
Backend API + Frontend: http://localhost:8080  
Optional frontend dev server: `cd web && npm install && npm run dev`
...
| `STORE_BACKEND` | No | `sqlite` | Storage backend (`sqlite`) |
